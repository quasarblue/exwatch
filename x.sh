#!/bin/bash
read -r CMC_API_KEY
curl -s -H "X-CMC_PRO_API_KEY: $CMC_API_KEY" -H "Accept: application/json" -d "" -G "https://pro-api.coinmarketcap.com/v1/exchange/map" | jq '.data[] | .id, .name' | awk '{id=$0; getline; name=$0; print id ";" name}'| sed 's/"//g' | awk 'BEGIN{FS=";"}{print $1}'> loop
while read id
do
